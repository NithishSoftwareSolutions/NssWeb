...
$conn = mysqli_connect(DB_HOST, DB_USERNAME,DB_PASSWORD, DB_DATABASE_NAME);
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(empty($_POST['mobileno'])) {
            ...
            $mobilequery = mysqli_query($conn, "select * from `internship_form` where mobileno='$mobileno'");
        ...
        if(empty($_POST['email'])) {
            $return_messages[] = "email_miss";
              ...
              $emailquery = mysqli_query($conn, "select * from `internship_form` where email='$email'");
