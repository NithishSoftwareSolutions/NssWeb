...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST['email'])) {
        ...
        $emailquery = mysqli_query($conn, "select * from `apply_details` where email='$email'");
    ...
    if (empty($number)) {
        ...
        $mobilequery = mysqli_query($conn, "select * from `apply_details` where number='$number'");
            if(mysqli_num_rows($mobilequery)>0) {
                $return_messages[] = "number_exists";
            }
    ...
    if ($_FILES['file']['name'] == "") {
        ...
        if ($ext != 'docx' && $ext != 'pdf') {
            ...
            if (move_uploaded_file($_FILES['file']['tmp_name'], $targetPath)) {
                ...
                $sql = "INSERT INTO apply_details (fname, lname, email, number, qualification, applyfor, file_path) 
    ...
    if (empty($return_messages)) {
        ...
        // Insert data into the database
        $sql = "INSERT INTO apply_details (fname, lname, email, number, qualification,applyfor,file_path) 
                VALUES ('$fname', '$lname', '$email', '$number', '$qualification','$applyfor','$file_path')";
